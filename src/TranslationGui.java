...
public class TranslationGui extends JFrame {
	...
	public void createView() {
		...
		// Hashtable of strings of full name of language for language iso codes
		Hashtable<String, String> langTable = new Hashtable<String, String>();

		String table[] = { "English", "Russian", "Spanish", "French", "Hindi", "German", "Greek", "Japanese", "Italian",
				"Polish", "Tagalog", "Vietnamese" };

		String languages[] = { "en", "ru", "es", "fr", "hi", "de", "el", "ja", "it", "pl", "tl", "vi" };

		int count = 0;

		while (count < languages.length) {
			langTable.put(table[count], languages[count]);
			count++;
		}
		...
		JComboBox<String> startLanguage = new JComboBox<>(table);

		startLanguage.setSelectedIndex(-1);

		JComboBox<String> endLanguage = new JComboBox<>(table);
		...
		/**
		 * Adding JButton with ActionListener that calls the Translation class's method
		 * to call and fetch a translation from the Google API URL.
		 *
		 */
		...
		// Uses input from the JComboBoxs and the text area to send to the translation
		// method.
		translateButton.addActionListener(ev -> {
			Translation translate = new Translation();
			try {
				textArea.setText(translate.parser(langTable.get(startLanguage.getSelectedItem().toString()),
						langTable.get(endLanguage.getSelectedItem().toString()), textArea.getText()));
				startLanguage.setSelectedIndex(-1);
				endLanguage.setSelectedIndex(-1);
			} catch (Exception e) {
				e.printStackTrace();
				JOptionPane.showMessageDialog(panel,
						"Please choose the appropriate 'Start' and 'End' Language Formats.");
			}
		});
